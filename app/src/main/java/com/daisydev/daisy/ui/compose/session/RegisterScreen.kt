...
import com.daisydev.daisy.ui.feature.session.RegisterViewModel
...
private fun Register(
    ...
    if (registerSuccess) {
        LaunchedEffect(Unit) {
            navController.navigate(NavRoute.Session.path)
        }
    } else if (isLoading) {
        LoadingIndicator()
    } else {
...
private fun ConditionsCheckbox(
    ...
    if (showDialog.value) {
        // Modal representation composable
        Dialog(
...
// Header of the registration screen
@Composable
